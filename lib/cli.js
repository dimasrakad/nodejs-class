...
_${crudFileName}.post = function (data, callback) {
  ...
  if (field1 && field2 && field3) {
    ...
    _data.create("${crudFileName}", field1, ${crudFileName.slice(
      0,
      -1
    )}Object, function (err) {
      if (!err) {
        ...
        console.log(err);
        callback(500, { Error: "Could not create new ${crudFileName.slice(
          0,
          -1
        )}" });
      }
...
_${crudFileName}.get = function (data, callback) {
  ...
  if (field1) {
    ...
    _data.read("${crudFileName}", field1, function (err, data) {
      if (!err && data) {
        callback(200, data);
      } else {
        callback(404);
...
_${crudFileName}.put = function (data, callback) {
  ...
  if (field1) {
    ...
    if (field2 || field3) {
      // Lookup the ${crudFileName.slice(0, -1)}
      _data.read("${crudFileName}", field1, function (err, ${crudFileName.slice(
      0,
      -1
    )}Data) {
        if (!err && ${crudFileName.slice(0, -1)}Data) {
          // Update the fields if necessary
          if (field2) {
            ${crudFileName.slice(0, -1)}Data.field2 = field2;
          }
          if (field3) {
            ${crudFileName.slice(0, -1)}Data.field3 = field3;
          }
          // Store the new updates
          _data.update("${crudFileName}", field1, ${crudFileName.slice(
      0,
      -1
    )}Data, function (err) {
            if (!err) {
              ...
              console.log(err);
              callback(500, { Error: "Could not update the ${crudFileName.slice(
                0,
                -1
              )}." });
            }
          ...
          callback(400, { Error: "Specified ${crudFileName.slice(
            0,
            -1
          )} does not exist." });
        }
...
_${crudFileName}.delete = function (data, callback) {
  ...
  if (field1) {
    ...
    _data.read("${crudFileName}", field1, function (err, data) {
      if (!err && data) {
        _data.delete("${crudFileName}", field1, function (err) {
          if (!err) {
            callback(200);
          } else {
            console.log(err);
            callback(500, { Error: "Could not delete the specified ${crudFileName.slice(
              0,
              -1
            )}" });
          }
        ...
        callback(400, { Error: "Could not find the specified ${crudFileName.slice(
          0,
          -1
        )}." });
      }
...
`,
    function (err) {
      if (err) throw err;
      console.log("File " + crudFileName + ".js is created successfully");
    }
  );
